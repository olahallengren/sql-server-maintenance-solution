...
BEGIN
  ...
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The value for the parameter @DatabasesInParallel is not supported. This option is not supported with file group backup.', 16, 5
  END
  ...
  BEGIN -- Start of database loop
    ...
    BEGIN
      IF ((@CurrentBackupType = 'FULL' AND @CopyOnly = 'N' AND @FileGroups IS NULL AND @Version >= 17)
      OR (@CurrentBackupType = 'DIFF' AND @CopyOnly = 'N' AND @FileGroups IS NULL AND @Version >= 17)
    ...
    BEGIN -- Start of database backup check
      ...
      WHILE (1 = 1)
      BEGIN -- Start of file group loop
